...
update currentDate backendUrl accessToken user msg model =
    case msg of
        ...
        Unsubscribe id ->
            ( { model | items = EveryDictList.remove id model.items }
            , Cmd.none
            , Nothing
            )
        ...
        MsgPagesItem id subMsg ->
            case getItem id model of
                Success item ->
                    ...
                    in
                        ( { model | items = EveryDictList.insert id (Success subModel) model.items }
        ...
        HandleFetchedItem itemId (Ok item) ->
            let
                ...
                updatedModel =
                    { model | items = EveryDictList.insert itemId (Success item) model.items }
            in
                ( updatedModel
                , Cmd.none
                , Nothing
        ...
        HandleFetchedItem itemId (Err err) ->
            ( { model | items = EveryDictList.insert itemId (Failure err) model.items }
            , Cmd.none
            , Nothing
            )
        ...
        HandlePusherEvent result ->
            case result of
                Ok event ->
                    case event.data of
                        ItemUpdate data ->
                            -- For now we just update the item in the items EveryDictList. But this
                            -- can be used to pipe the pushed data to child components.
                            ( { model | items = EveryDictList.insert event.itemId (Success data) model.items }
                            , Cmd.none
                            , Nothing
                            )
...
fetchItemFromBackend backendUrl accessToken itemId model =
    let
        ...
        command =
            HttpBuilder.get (backendUrl ++ "/api/items/" ++ itemIdValue)
                |> withQueryParams [ ( "access_token", accessToken ) ]
                |> sendWithHandler decodeItemFromResponse (HandleFetchedItem itemId)
    in
        ( { model | items = EveryDictList.insert itemId Loading model.items }
        , command
        )
